...
export const MyPageWithdrawMain = () => {
  ...
  const withdrawHandler = () => {
    if (withdrawModal === false) setWithdrawModal(true);
  };

  // ESC=닫기 작동
  const pressEscHandler = (event) => {
    if (event.key === "Escape") setWithdrawModal(false);
  };
    ...
    <>
      ...
      <>
        <NoticeContainer onKeyDown={pressEscHandler}>
          ...
          <Button onClick={withdrawHandler} style={{ width: "100px" }}>
            확인
          </Button>
        </NoticeContainer>
        {withdrawModal !== false && <MyPageWithdrawModal />}
      </>
    </>
  );
};
