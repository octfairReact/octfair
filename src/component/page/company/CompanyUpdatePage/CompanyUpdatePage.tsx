...
import { CompanyUpdatePageStyled } from "./styled";
...
export const CompanyUpdatePage = () => {
  ...
  const [postDetail, setPostDetail] = useState<ICompanyDetail>();
  ...
  useEffect(() => {
    companyGetInfo(userId);
  }, [userId]);

  const companyGetInfo = async (userId: string) => {
    try {
      // 회사 정보 가져오기
      const companyInfo = await postCompanyApi<ICompanResponse>(Company.getCompanyInfo, { userId });
      console.log("받아온 값", companyInfo.data.detail); // 응답 데이터를 콘솔에 출력하거나 상태에 저장
      setPostDetail(companyInfo.data.detail);
      setCompanyInfo({
        // 입력 필드에 연결할 상태 업데이트
        bizName: companyInfo.data.detail.bizName || "",
        bizCeoName: companyInfo.data.detail.bizCeoName || "",
        bizContact: companyInfo.data.detail.bizContact || "",
        bizAddr: companyInfo.data.detail.bizAddr || "",
        bizIntro: companyInfo.data.detail.bizIntro || "",
        bizEmpCount: String(companyInfo.data.detail.bizEmpCount || ""),
        bizWebUrl: companyInfo.data.detail.bizWebUrl || "",
        bizFoundDate: companyInfo.data.detail.bizFoundDate || "",
        bizRevenue: companyInfo.data.detail.bizRevenue || "",
        bizLogo: companyInfo.data.detail.bizLogo || "",
      });
      console.log("setCompanyInfo", companyInfo.data.detail.bizLogo);
      const { fileExt, logicalPath } = companyInfo.data.detail;
      ...
      console.error("회사를 가져오는 데 실패했습니다:", error);
    }
  };
  ...
  const handlerUpdate = async (e: React.FormEvent) => {
    ...
    const update = await postCompanyApi<IPostResponse>(Company.postUpdate, fileForm);
  ...
  return (
    <div>
      <CompanyUpdatePageStyled>
        <form id="companySaveForm" onSubmit={handlerUpdate}>
          <div id="companyWriteSt" style={{ marginTop: "10px" }}>
            <div id="writeTable">
              ...
              <div className="btn_areaC mt30">
                <button type="submit" className="btnType blue" id="btnCompanyUpdate">
                  <span>수정</span>
                </button>
                <button className="btnType blue" id="btnCompanyUpdate" onClick={CompanyDelete}>
                  <span>삭제</span>
                </button>
