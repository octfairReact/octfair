...
export const HistoryModal = ({
  ...
  const getResumeInfo = historyData?.resumeInfo || resumeInfo;          // API로 받아온 이력서 정보가 있을 경우 그 정보를, 없으면 기본 정보를 사용
  const getSkillInfo = historyData?.skillInfo?.length > 0 ? historyData.skillInfo : skillInfo;    // 스킬 정보가 있으면 그것을, 없으면 기본 스킬 정보를 사용  
  const getCertInfo = historyData?.certInfo?.length > 0 ? historyData.certInfo : certInfo;        // 자격증 정보가 있으면 그것을, 없으면 기본 자격증 정보를 사용
      ...
      <div>
        ...
        <table className="cert-table">
          ...
          <tbody>
            <tr>
              <td>이름</td>
              <td>{getResumeInfo.userNm ? getResumeInfo.userNm : "정보 없음"}</td>
            </tr>
            <tr>
              <td>이메일</td>
              <td>{getResumeInfo.email ? getResumeInfo.email : "정보 없음"}</td>
            </tr>
            <tr>
              <td>연락처</td>
              <td>{getResumeInfo.phone ? getResumeInfo.phone : "정보 없음"}</td>
            </tr>
          </tbody>
        </table>
